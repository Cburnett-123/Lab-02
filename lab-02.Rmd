...
author: "Kate Supremacy: Connor, Kate, Ruth, Kate and Torgua"
date: "`r Sys.Date()`"
...
plastic_data_all %>% 
  ...
  group_by(region) %>% 
  summarise(mean_mismanaged_plastic = mean(total_mismanaged_plastic, na.rm = TRUE))
#Asia
#Oceania
```
...
plastic_data_all %>% 
  group_by(region) %>% 
  summarise(median_mismanaged_plastic = median(pct_mismanaged_plastic_ocean))
...
#(plastic_data_all <- plastic_data_all %>% 
 # rename(
  #  reg = region,
   # pop = population,
    #mis_ocean = mismanaged_plastic_ocean,
    #mis_plas = mismanaged_plastic, 
    #pct_ocean = pct_mismanaged_plastic_ocean
 # ))
  
  plastic_data_all %>% 
 str_replace_all( "Latin America and The Caribbean", "LAC")
